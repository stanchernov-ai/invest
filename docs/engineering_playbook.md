...
- **Watchlist Pass cooldown is cross-run state, not repo artifacts** — `src/verdict_memory.py` appends chairman watchlist **Pass** entries to `board_verdicts.json` at end of deliver **only when `debate.is_approved`** (Markopolos gate). Scout reads that file + CSV-owned tickers (`run_scout_pipeline(owned_tickers=…)` after `process_portfolios`). Post-flight QA does **not** gate persistence (by design — keep simple). Removed dead `save_memory()` / `save_verdict_history()` / `ledger_state.json`. Generated run markdown/HTML stays gitignored. (2026-05-29)
...
- **Latest deploy on `main`:** `29b4101` (May 28, 2026) atop `4dc0b2e` — governance tooling + benchmark scrub utility. Verify on first post-deploy pipeline run before picking new backlog. (2026-05-28)
