...
- **Graphics QA is deterministic (no LLM)** — `qa_pipeline.build_graphics_report` turns the chart-health HTTP probe directly into the report; the LLM graphics reviewer was the first non-crucial QA cut. **QA Integrity runs on Flash + a hard 90s timeout** (`qa_pipeline.run_qa_integrity_audit`); on timeout it emits a non-blocking WARNING, never a false fail or a ceiling breach. (2026-05-28)
