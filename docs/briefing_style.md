...
## Email-safe rendering (Gmail / Outlook)

Email clients strip ``<head>`` CSS and ignore CSS variables. The briefing uses **dual delivery**:

1. **Inline styles + ``bgcolor``** on wrapper tables — `executive_briefing_inline_styles()` in `briefing_style.py` (primary for inbox)
2. **``<head>`` CSS block** — `executive_briefing_css()` (fallback for browser preview / QA dashboard parity)

Wrapper structure:

```html
<body style="background-color:#121212; ...">
  <table role="presentation" bgcolor="#121212" style="background-color:#121212;">
    <tr><td>
      <table width="800" bgcolor="#1e1e1e" style="background-color:#1e1e1e;">
        <tr><td style="padding:30px; color:#a1a1aa;"> ... content ... </td></tr>
      </table>
    </td></tr>
  </table>
</body>
...
Metric boxes, chart cards, hedge/red-team blocks, and headings all carry inline hex colors — never rely on class-only rules for the investor email.
